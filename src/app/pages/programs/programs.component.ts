...
export class ProgramsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.route.queryParamMap.subscribe(params => {
      if (params) {
        ...
        const STATE_KEY = makeStateKey<any>('programs'); // create a key for saving/retrieving a state
  ...
  updateMeta() {
    ...
    if (this.filters.params.category) {
      this.titleService.setTitle(`Online Coaching Programs on ${cat}`);
      this.metaTagService.updateTag({name: 'description', content: `Top rated online coaching programs on ${cat.toLowerCase()}`});
    }
  }
  ...
  receivePageUpdate(event: number) {
    ...
    // console.log('programs component received new page request:', event);
  ...
  async getSearchResults() {
    const res = await this.searchService.searchPrograms(this.hitsPerPage, this.page, this.filters);
  ...
  ngOnDestroy() {
    ...
    const body = this.document.getElementsByTagName('body')[0];
    body.classList.remove('programs-page');
  }
